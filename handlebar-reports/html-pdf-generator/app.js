...
let getHandlerBarTemplate = () => readFile(templatePath).then(compileBufferToTemplate);
let generateHtml = ([ data, template ]) => template(data);
let saveFile = (data) => {
	writeFile('./html/Report2020.html', data);
	return data;
};
let pdfGenerater = (data) => htmlPdf.create(data).then((pdf) => pdf.toFile('./pdf/Report2020.pdf'));
